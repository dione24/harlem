...
class PiecesManagerPDO extends PiecesManager {
    ...
    public function deleteStock($stock) {
        ...
        $requete->bindValue(':id',$stock,\PDO::PARAM_INT);
        $requete->execute();
    }
    public function quantiteRestante($piece) {
        ...
        $requete_in->bindValue(':id',$piece,\PDO::PARAM_INT);
        $requete_in->execute();
        ...
        $requete_out->bindValue(':id',$piece,\PDO::PARAM_INT);
        $requete_out->execute();
    ...
    public function ventePieces($vente) {
        ...
        $requete->bindValue(':id',$vente,\PDO::PARAM_INT);
        $requete->execute();
        $resultat = $requete->fetchAll();
        return $resultat;
    }
    public function addVente($vente) {
        ...
        $requete->bindValue(':date',$vente->postData('date'),\PDO::PARAM_INT);
        $requete->bindValue(':nom',$vente->postData('nom'),\PDO::PARAM_INT);
        $requete->bindValue(':prenom',$vente->postData('prenom'),\PDO::PARAM_INT);
        $requete->bindValue(':telephone',$vente->postData('telephone'),\PDO::PARAM_INT);
        $requete->execute();
    ...
    public function countFctreImp() {
        ...
        public function GetListeVente($vente) {
        $requete = $this->dao->prepare("SELECT * FROM tblpieces INNER JOIN tblstockpieces ON (tblpieces.RefPieces = tblstockpieces.RefPieces) INNER JOIN tblvente ON (tblstockpieces.RefVente = tblvente.RefVente) WHERE tblvente.RefVente=:id AND tblstockpieces.RefTypeOperation=2
");
        $requete->bindValue(':id',$vente,\PDO::PARAM_INT);
        $requete->execute();
        $resultat = $requete->fetchAll();
