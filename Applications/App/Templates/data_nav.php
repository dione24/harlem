<?php
  ...
  function nbreMaintenanceDans4J($connexion) {
    $requeteMaintenance = $connexion->prepare("SELECT * FROM tblmaintenance INNER JOIN tblstatutreparation ON (tblmaintenance.RefStatut = tblstatutreparation.RefStatut) WHERE tblmaintenance.Date>=:today AND tblmaintenance.Date<=:plus4");
